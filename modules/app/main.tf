...
resource "terraform_data" "build" {
  ...
  provisioner "local-exec" {
    command = <<EOT
    docker build -t  ${local.ecr_url} ${path.module}/apps/ui/${var.app_path}
    EOT
  }
}
...
resource "terraform_data" "push" {
  ...
  provisioner "local-exec" {
    ...
    docker image push ${local.ecr_url} ${local.ecr_url}:${var.image_version}
    docker image push ${local.ecr_url} ${local.ecr_url}:latest
    EOT
  }
}
